...
use crate::{AmsAddr, AmsNetId, Client, Device, Source};
...
fn run_test(opts: ServerOpts, f: impl Fn(Device)) {
    ...
    let (client, reader) = Client::new(("127.0.0.1", port), timeouts, Source::Auto).unwrap();
...
fn test_readwrite() {
    run_test(ServerOpts::default(), |device| {
        ...
        assert!(matches!(
            ...
            Err(Error::API(_, 0x702))
        ));
        assert!(matches!(
            device.read_exact(0x4020, 98765, &mut buf),
            Err(Error::API(_, 0x703))
        ));

        device.write_value(0x4020, 7, &0xdead_beef_u32).unwrap();
        assert!(device.read_value::<u32>(0x4020, 7).unwrap() == 0xdead_beef);
    });
}

#[test]
fn test_multi_requests() {
    use crate::client::{ReadRequest, WriteReadRequest, WriteRequest};
    use crate::index::{FILE_CLOSE, FILE_OPEN, FILE_READ};
    run_test(ServerOpts::default(), |device| {
        let mut buf1 = *b"ABCDEFGHIJ";
        let mut buf2 = *b"0123456789";
        let mut buf3 = *b"-----";
        let mut reqs = vec![
            WriteRequest::new(0x4020, 7, &buf1).unwrap(),
            WriteRequest::new(0x4020, 9, &buf2).unwrap(),
            WriteRequest::new(0x6789, 5, &buf3).unwrap(),
        ];
        ...
        let mut reqs = vec![
            ReadRequest::new(0x4020, 7, &mut buf1).unwrap(),
            ReadRequest::new(0x4020, 9, &mut buf2).unwrap(),
            ReadRequest::new(0x7689, 5, &mut buf3).unwrap(),
        ];
        ...
        let mut reqs = vec![
            WriteReadRequest::new(FILE_OPEN, 0, b"/etc/passwd", &mut buf1).unwrap(),
            WriteReadRequest::new(FILE_READ, 0, b"", &mut buf2).unwrap(),
            WriteReadRequest::new(FILE_CLOSE, 42, b"", &mut []).unwrap(),
            WriteReadRequest::new(0x7689, 5, b"blub", &mut buf3).unwrap(),
        ];
...
fn test_symbolaccess() {
    ...
    run_test(ServerOpts::default(), |device| {
        ...
        handle.write_value(&0xdead_beef_u32).unwrap();
        assert!(handle.read_value::<u32>().unwrap() == 0xdead_beef);
    });
}

#[test]
fn test_notification() {
    use crate::notif::{Attributes, Sample, TransmissionMode};
    run_test(ServerOpts::default(), |device| {
        ...
        let ts = Timestamp::from_nanos(1_234_567_890_123_456_789 / 100 * 100);
...
fn test_multi_notification() {
    use crate::client::{AddNotifRequest, DelNotifRequest};
    use crate::notif::{Attributes, TransmissionMode};
    run_test(ServerOpts::default(), |device| {
        let attrib = Attributes::new(
            4,
            TransmissionMode::ServerOnChange,
        ...
        let mut reqs = [
            AddNotifRequest::new(0x4020, 7, &attrib).unwrap(),
            AddNotifRequest::new(0x6789, 0, &attrib).unwrap(),
        ];
...
fn test_wstring_type() {
    ...
    run_test(ServerOpts::default(), |device| {
        ...
        assert!(
            <[u16; 5]>::from(wstr) == [u16::from(b'a'), u16::from(b'b'), u16::from(b'c'), 0, 0]
        );
        assert!(wstr == &[u16::from(b'a'), u16::from(b'b'), u16::from(b'c')][..]);
        assert!(wstr == "abc");
        assert!(wstr.len() == 3);
        ...
        let wstr2 =
            WString5::try_from(&[u16::from(b'a'), u16::from(b'b'), u16::from(b'c')][..]).unwrap();
        assert!(
            <[u16; 5]>::from(wstr2) == [u16::from(b'a'), u16::from(b'b'), u16::from(b'c'), 0, 0]
        );
        assert!(wstr == wstr2);
        ...
        assert!(<[u16; 5]>::from(ret) == [u16::from(b'a'), u16::from(b'b'), u16::from(b'c'), 0, 0]);
        assert!(String::try_from(ret).unwrap() == "abc");
        assert!(<Vec<u16>>::from(ret) == [u16::from(b'a'), u16::from(b'b'), u16::from(b'c')]);
    });
}
