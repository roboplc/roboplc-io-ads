...
fn udp_replier(sock: UdpSocket) {
    ...
    loop {
        ...
        if udp::Message::parse(&buf[..n], udp::ServiceId::Identify, false).is_ok() {
            ...
            reply.add_str(udp::Tag::ComputerName, "box").unwrap();
            reply.add_bytes(udp::Tag::OSVersion, OS_VERSION).unwrap();
            reply
                .add_bytes(udp::Tag::TCVersion, b"\x04\x01\x07\x00")
                .unwrap();
            ...
            reply.add_u32(udp::Tag::Status, u32::from(status)).unwrap();
        } else {
