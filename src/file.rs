...
impl File {
    ...
    pub fn open(device: &Device, filename: impl AsRef<[u8]>, flags: u32) -> Result<Self> {
        let mut hdl = [0; 4];
    ...
    loop {
        match device.write_read_exact(index::FILE_BROWSE, offset, &argument, &mut buf) {
            Ok(()) => {
                ...
                let size_h = LE::read_u32(&buf[32..36]);
                let size_l = LE::read_u32(&buf[36..40]);
                let size = (u64::from(size_h)) << 32 | u64::from(size_l);
                let name = buf[48..].iter().copied().take_while(|&b| b != 0).collect();
                files.push((name, attrs, size));
            }
            // Error "not found" means the end of the list.
            Err(Error::API(_, 0x70c)) => return Ok(files),
            Err(e) => return Err(e),
        }
...
fn map_error(e: Error) -> io::Error {
    match e {
        Error::API(_, 0x704) => io::ErrorKind::InvalidInput.into(),
        Error::API(_, 0x70C) => io::ErrorKind::NotFound.into(),
